...
yum -y install libselinux-utils policycoreutils policycoreutils-python-utils sed selinux-policy selinux-policy-base selinux-policy-targeted
# pre-requisites for docker on RHEL
yum -y install device-mapper libseccomp libtool-ltdl libcgroup iptables

#yum -y install /tmp/docker-ce/*.rpm
...
yum -y install ./containerd.io-1.6.9-3.1.el8.x86_64.rpm
