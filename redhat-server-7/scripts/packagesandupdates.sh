...
#yum -y install /tmp/epel-release-latest-8.noarch.rpm

yum -y install python3-policycoreutils selinux-policy-devel openldap-clients sssd sssd-ldap authconfig pam_ldap sssd-ad nfs-utils
yum -y install realmd adcli sssd oddjob oddjob-mkhomedir samba-common-tools krb5-workstation authselect-compat
...
yum -y install perl open-vm-tools openssh chrony system-storage-manager #ntp

#packages needed for Ansible
yum -y install git
yum -y install python3-pip # python39 Probably do not need Python 3.9
#packaged needed for CIS hardening playbook
yum -y install dconf audit cronie authconfig
