...
source "vsphere-iso" "linux-redhat-server8" {
  ...
  vm_name              = "${var.vm_guest_os_family}-${var.vm_guest_os_vendor}-${var.vm_guest_os_member}"
...
#https://serverfault.com/questions/1049693/oel8-kickstart-install-on-esxi-hangs-at-reached-target-basic-system-why
 cd_files = ["./configs/ks-rhel8.cfg"] 
 cd_label = "autoinstall"
  boot_command = [
    "<up>",
    "e",
    "<down><down><end><wait>",
    "vmlinuz initrd=initrd.img ip=10.106.68.120::10.106.68.1:255.255.255.0:pranchrhel8tmp:ens224:none nameserver=10.0.3.72  nameserver=10.0.3.71 text inst.ks=cdrom:/ks-rhel8.cfg",
#    "vmlinuz initrd=initrd.img ip=10.106.68.120::10.106.68.1:255.255.255.0:pranchrhel8tmp:ens224:none nameserver=10.0.3.72  nameserver=10.0.3.71 text inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/${var.http_file}",
...
build {
  sources = ["source.vsphere-iso.linux-redhat-server8"]
  /*
...
#  }
  provisioner "file" {
    destination = "/tmp/linux-install-ama.sh"
    source = "scripts/linux-install-ama.sh"
  }
  provisioner "file" {
  ...
  provisioner "file" {
    destination = "/tmp/flexera"
    source = "../../../configfiles/flexera"
  }
  provisioner "file" {
  ...
  provisioner "file" {
    destination = "/tmp"
    source = "../../../configfiles/acdcs_sudoers/"
  }
#  provisioner "file" {
...
#  }
  ...
  provisioner "shell" {
    ...
    scripts = [
      ...
      "scripts/disable_ctrl_alt_delete.sh",
      "scripts/chrony.sh",
      "scripts/oracle-preinstall.sh",
      "scripts/oracle-sftp-install.sh",
      "scripts/reboot.sh"
    ]
  }
  // Uses the Shell Provisioner to execute a series of inline commands defined in the variables
  provisioner "shell" {
    ...
    #  "scripts/goss.sh",
      ...
      "scripts/oracle-acdcs-accounts.sh",
      "scripts/oracle-acdcs-sudoers.sh",
    ...
    #  "scripts/linux-install-oms.sh",
     ...
     # "scripts/flexera_install.sh",
      "scripts/harden.sh",
     # "scripts/linux-install-ama.sh",
