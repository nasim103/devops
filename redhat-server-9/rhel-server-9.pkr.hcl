...
source "vsphere-iso" "linux-redhat-server9" {
  ...
  notes                = "Built by HashiCorp Packer on ${local.buildtime}. Development v1.0.0"
...
#https://serverfault.com/questions/1049693/oel8-kickstart-install-on-esxi-hangs-at-reached-target-basic-system-why
 cd_files = ["./configs/ks-rhel9.cfg"] 
 cd_label = "autoinstall"
  boot_command = [
    "<up>",
    "e",
    "<down><down><end><wait>",
    "vmlinuz initrd=initrd.img ip=10.106.68.120::10.106.68.1:255.255.255.0:pranchrhel9tmp:ens224:none nameserver=10.0.3.72  nameserver=10.0.3.71 text inst.ks=cdrom:/ks-rhel9.cfg",
...
build {
  sources = ["source.vsphere-iso.linux-redhat-server9"]
  /*
...
#  }
  ...
  provisioner "shell" {
    ...
    scripts = [
      ...
      "scripts/selinux.sh",   
      "scripts/rclocal.sh",
      "scripts/harden_prereqs1.sh",
      "scripts/harden_prereqs2.sh",
      "scripts/harden_prereqs2.sh",
