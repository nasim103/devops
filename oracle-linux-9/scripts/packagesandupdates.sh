...
dnf -y install python3-policycoreutils selinux-policy-devel openldap-clients sssd sssd-ldap authconfig  sssd-ad nfs-utils # pam_ldap not in 9
...
dnf -y install perl open-vm-tools openssh chrony  # system-storage-manager not in 9

#packages needed for Ansible
dnf -y install git
dnf -y install python3-pip # python39 Probably do not need Python 3.9
#packaged needed for CIS hardening playbook
dnf -y install dconf audit cronie authconfig aide
