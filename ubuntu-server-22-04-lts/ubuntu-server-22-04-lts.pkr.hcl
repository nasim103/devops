# 
# Packer tenmplate for Ubuntu Server 22.04 LTS.
...
source "vsphere-iso" "linux-ubuntu-server" {
  ...
  cd_files = ["./configs/meta-data","./configs/user-data"]
  cd_label                  = "cidata"
  boot_command= [ 
    "<esc><wait>",
    "e<wait>",
    "<down><down><down><end>",
    "<bs><bs><bs><bs><wait>",
   // "ip=10.106.68.120::10.106.68.1:255.255.255.0::::10.0.3.72  autoinstall ds=nocloud\\;seedfrom=cidata ---<wait>",
    "ip=10.106.68.120::10.106.68.1:255.255.255.0::::10.0.3.72  autoinstall ds=nocloud\\ ---<wait>",
    "<f10><wait>",
  ]
  ip_wait_timeout           = "20m"
